...
Before starting the tutorial, the first thing I need to explain is what a “Install” actually is. An install in most games originates from fighting games, when a Player character will build up a resource, normally in the form of a meter, to enter a new “state”. This state will change what set of abilities are available to the player whilst using the same buttons (i.e. the player will get result “a” when they click “a”, but when in install, they will get result “b” when clicking “a” instead.).

To illustrate this, in this Tutorial we will need some items to illustrate this better:

1.	A way to represent what value we are at for the install.
2.	A way to show in this tutorial, if we are in install.
3.	A way to show the different changes whilst in install.
...
The first thing we want to do is to write a script for our install code. My advice is to go into assets and right click, and hit folder. We can call this “InstallSystem”. This is where we can keep all the assets for our items involved with managing the install system. Double click this folder to enter it, and right click it again, this time selecting C# Script and naming it “InstallMechanic”.
...
At the very top, we start with a Boolean. This is a variable that can only be either true or false. This is what we will assign our install to being at the start, turning it off for our player. The next two variables our floats, which are values of integers that can be decimals. Here we will make two variables, the first being maxInstall to tell our program what the max value of the install cane be, and below that, we will make the installValue, which will change its value as we charge the install, and as it depletes.
...
1.	idleColour for idle state (when the Player is not in install)
2.	attackColour for attack state (when the Player is not in install and attacking)
3.	installColour for install state (when the Player is in Install)
4.	installAttackColour for install attack state (when the Player is in install and attacking)
...
The last thing we need to do is repeat this process but if the Player is attacking for both no Install and true Install.
...
This code focuses on changing the value of instal. The first is a flat increase and the second is a decrease over time. We haven’t assigned these values yet but we will assign a separate a value to them. For now, this code also adds a limit to how much and how little they decrease by, as well as setting the meter to change as well.
...
Now, go back to the Install mechanic script and drag the Install object from the canvas in hierarchy to the Install Slider in the Install Mechanic Script and it should look like this.
